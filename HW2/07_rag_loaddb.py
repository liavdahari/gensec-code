...
def load_docs(docs):
    ...
    # 1. Decrease chunk size to standard RAG lengths to improve accuracy and reduce tokens
    text_splitter = RecursiveCharacterTextSplitter(chunk_size=1500, chunk_overlap=150)
    splits = text_splitter.split_documents(docs)
    
    # 2. Explicitly send documents to the database in smaller batches
    # 20 chunks * ~1500 chars = ~30,000 chars (approx. 7,500 tokens) 
    # This safely stays well under the 20,000 token Vertex AI limit
    batch_size = 20
...
def load_notes(directory):
    """Load structured research notes from a directory using custom ResearchNoteLoader."""
    if os.path.exists(directory):
        loader = ResearchNoteLoader(directory, glob_pattern="**/*.md")
        load_docs(loader.load())

def load_json(directory):
    """Load structured JSON datasets from a directory using custom ResearchNoteLoader."""
    if os.path.exists(directory):
        loader = ResearchNoteLoader(directory, glob_pattern="**/*.[jJ][sS][oO][nN]")
        load_docs(loader.load())
...
notes_directory = "rag_data/notes"
print(f"Loading custom Research Notes from: {notes_directory}")
load_notes(notes_directory)

json_directory = "rag_data/JSON"
print(f"Loading custom JSON datasets from: {json_directory}")
load_json(json_directory)
